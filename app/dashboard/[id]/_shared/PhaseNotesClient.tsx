...
export default function PhaseNotesClient({
  ...
  const [items, setItems] = useState<Record<string, Record<string, string[]>>>(
    () => Object.fromEntries(stepDetails.map((s) => [s.key, {}]))
  );
  ...
  const [saveStates, setSaveStates] = useState<Record<string, SaveState>>(() =>
    Object.fromEntries(stepDetails.map((s) => [s.key, "idle" as SaveState]))
  );
  ...
  // Teilnehmer + Notizen laden
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(
          `/api/mediations/${mediationId}/participants`
        );
        if (!res.ok) return;
        ...
        const notesResults = await Promise.all(
          stepDetails.map((step) =>
            fetch(
              `/api/mediations/${mediationId}/notes?phase=${step.key}`
            )
              .then((r) => (r.ok ? r.json() : []))
              .then(
                (
                  notes: { participant_id: string; content: string }[]
                ) => ({ key: step.key, notes })
              )
          )
        );
  ...
  function addItem(stepKey: string, participantId: string) {
  ...
  function removeItem(stepKey: string, participantId: string, index: number) {
    setItems((prev) => ({
      ...prev,
      [stepKey]: {
        ...prev[stepKey],
        [participantId]: (prev[stepKey]?.[participantId] ?? []).filter(
          (_, i) => i !== index
        ),
      },
    }));
  ...
  async function saveStep(stepKey: string) {
    ...
    try {
      const content = JSON.stringify(
        items[stepKey]?.[currentParticipant.id] ?? []
      );
      ...
      setTimeout(
        () => setSaveStates((prev) => ({ ...prev, [stepKey]: "idle" })),
        2500
      );
    } catch {
      ...
      <section className="container py-12">
        ...
        <div className="app-surface p-8">
          <p className="eyebrow mb-3">
            Phase {currentIndex + 1} von {PHASES.length}
          </p>
          ...
          <div className="mt-10 space-y-12">
            {stepDetails.map((step, stepIndex) => {
              const ss = saveStates[step.key] ?? "idle";
              return (
                <div key={step.key}>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    {accepted.map((participant) => (
                      <ParticipantCard
                        ...
                        onRemove={(idx) =>
                          removeItem(step.key, participant.id, idx)
                        }
                      />
                    ))}
                  </div>
          ...
          <div className="mt-8 flex items-center justify-between pt-2">
            <button
              ...
              onClick={() =>
                phase.prevPhase
                  ? router.push(
                      `/dashboard/${mediationId}/${phase.prevPhase}`
                    )
