...
// Öffentliche Privatpersonen-Registrierung (keine Domain-Beschränkung).
export async function POST(request: NextRequest) {
  try {
    ...
    if (!res.ok) {
      return NextResponse.json({ error: data?.detail ?? "Registrierung fehlgeschlagen" }, { status: res.status });
    }
